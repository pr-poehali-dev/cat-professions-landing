      ...
      <div className="container mx-auto">
        ...
        <div className="mt-12 text-center">
          ...
          <p className="mt-4 text-sm text-gray-500 italic">
            * Все термины одобрены Международной Ассоциацией Профессиональных Котов (МАПК)
          </p>
        </div>
      </div>
    </section>
...
const TermCard = ({ category, term, definition, example }: TermCardProps) => {
  return (
    <div className="bg-[#F5F0E1] p-6 rounded-lg shadow-md relative overflow-hidden group hover:shadow-xl transition-all duration-300">
      <span className="absolute top-0 right-0 bg-[#D35536] text-white px-3 py-1 text-xs font-bold">{category}</span>
      <h3 className="text-xl font-bold mb-2">{term}</h3>
      <p className="text-gray-700"><span className="font-bold">Определение:</span> {definition}</p>
      <div className="mt-3 text-sm text-gray-500">
        <p><span className="font-bold">Пример:</span> "{example}"</p>
      </div>
